...
//! Both work on any type that derives [`std::fmt::Debug`],
//! [`serde::Serialize`], and [`SpytialDecorators`]. Use [`ViewerSession`] with
//! [`dbg_in!`] when one process needs separate, named capture streams.
//!
...
#[doc(hidden)]
pub fn __capture_dbg<T: spytial_annotations::HasSpytialDecorators + Serialize>(
    value: &T,
    expression: &str,
    file: &str,
    line: u32,
...
#[doc(hidden)]
pub fn __capture_dbg_in<T: spytial_annotations::HasSpytialDecorators + Serialize>(
